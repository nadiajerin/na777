...
export default function ChickenRoadGame() {
  ...
  // CashOut Updated Data
  async function betUpdateDatabase (updateAmout) {
    const updateResponse = await fetch(`/api/info/${user.email}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(updateAmout),
    });
  ...
  const startGame = () => {
    ...
    const updateAmout = balance - betAmount;
    betUpdateDatabase(updateAmout)
  };
  ...
  // CashOut Updated Data
  async function cashOutDatabase (updateAmout) {
    const updateResponse = await fetch(`/api/info/${user.email}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
  ...
  const cashOut = () => {
    ...
    const updateAmout = balance + winAmount;
    // console.log(updateAmout)
    cashOutDatabase(updateAmout)
    setIsPlaying(false);
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end space-y-4 sm:space-y-0 sm:space-x-6">
            <button
              ...
              className={isPlaying ? "bg-green-500 hover:bg-green-600 text-black  font-bold px-8 py-3 rounded text-lg w-full sm:w-auto" :
                 "bg-gray-500 text-black  font-bold px-8 py-3 rounded text-lg w-full sm:w-auto"}
              disabled={!isPlaying}
            >
              GO
            </button>
