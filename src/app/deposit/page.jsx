...
const page = () => {
    ...
    const handleDepositBkash = async (e) => {
        e.preventDefault()
        const form = e.target;
        const trans = form.trans.value;
        const number = form.number.value;

         // Now post the user data into database
         const response = await fetch('/api/deposit/bkash', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({ trans, number, email: user.email})
          });
    
          const data = await response.json();  // <- NEED to await here
        //   console.log('Server Response:', data);
        if(data.insertedId){
            alert("Your Deposit Request Sent")
        }
    }
    ...
    const handleDepositBNagad = async (e) => {
        ...
        const number = form.number.value;
         ...
         // Now post the user data into database
         const response = await fetch('/api/deposit/nagad', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
    ...
    return (
        <div>
            <Navbar />
            <p className='bg-yellow-400  mx-auto md:w-2/3 mt-4 md:mt-10 text-center p-1'>ডিপোজিট</p>
                        ...
                        <div className="modal-box p-4 ">
                            ...
                            <form onSubmit={handleDepositBkash}>
                                <div className='mt-2 md:mt-10'>
                                    ...
                                    <input name='trans' type="text" className='border-2 px-4 py-2 border-red-600' required/>
                                </div>
                                <div className='mt-2'>
